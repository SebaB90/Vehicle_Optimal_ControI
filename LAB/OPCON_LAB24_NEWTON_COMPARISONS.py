...
def cost_fcn(xx):
    ...
    """
    ll = np.exp(xx[0] + 3*xx[1]-0.1) + np.exp(xx[0]-3*xx[1]-0.1) + np.exp(-xx[0]-0.1)
    # Compute gradient components 
    dl1 = np.exp(xx[0] + 3*xx[1] - 0.1) + np.exp(xx[0] - 3*xx[1] - 0.1) - np.exp(-xx[0] -0.1)
    dl2 = 3.*np.exp(xx[0] + 3*xx[1] - 0.1) - 3*np.exp(xx[0] - 3*xx[1] - 0.1)

    d2l11 = np.exp(xx[0] + 3*xx[1] - 0.1) + np.exp(xx[0]-3*xx[1]-0.1) + np.exp(-xx[0]-0.1)
...
def min_cvx_solver(xx):
    ...
    cost = cvx.exp(xx[0] + 3*xx[1]-0.1) + cvx.exp(xx[0]-3*xx[1]-0.1) + cvx.exp(-xx[0]-0.1)
